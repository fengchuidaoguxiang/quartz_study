...
//        scheduler.getListenerManager().addTriggerListener(new MyTriggerListener("defined TriggerName"),EverythingMatcher.allTriggers());
        //创建并注册一个局部的TriggerListener
        scheduler.getListenerManager().addTriggerListener(new MyTriggerListener("simple2"),KeyMatcher.keyEquals(TriggerKey.triggerKey("trigger1","triggerGroup1")));
        //启动
        scheduler.start();
        //关闭
//      scheduler.shutdown();
